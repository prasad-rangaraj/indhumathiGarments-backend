...
export default async function uploadPublicRoutes(app: FastifyInstance) {
  // Upload route — Customer/Public, stores image privately on S3
  app.post('/upload', { preHandler: [protect] }, async (request, reply) => {
    try {
      ...
      // Build a clean S3 key: public-uploads/<timestamp>-<sanitized-filename>
      const sanitized = data.filename.replace(/[^a-zA-Z0-9.\-_]/g, '');
      const key = `public-uploads/${Date.now()}-${sanitized}`;
